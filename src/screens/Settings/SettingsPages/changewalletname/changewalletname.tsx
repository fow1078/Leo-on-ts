...
const ChangeWalletName: React.FC = () => {
	const { t } = useTranslation();              
  const { setAddress } = useStore();
  const [addOrBackBtn, setAddOrBackBtn] = useState('wallet_name_edit');
  let WALLET_NAME: any;
  return (
    <>
      {addOrBackBtn === 'wallet_name_edit' && 
        <Container>
          ...
          <div style={{display: 'flex', justifyContent: 'start', alignItems: 'center', width: '100%', position: 'relative'}}>
            ...
            <ItemTitle>{t('edit_name')}</ItemTitle>
          </div>
          <AddNewMenuContainer>
            <WrapperSubTitle>Enter name for your wallet</WrapperSubTitle>
            <AddNewInput type={'text'} placeholder={'Enter name'} onInput={(e) => {let WALLET_TEMP_NAME: any = e.target; WALLET_NAME = WALLET_TEMP_NAME.value }}/>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
              ...
              <Button style={{ width: '150px', backgroundColor: '#285BFE', borderRadius: '10px' }} label={t('save')} onPress={() => {window.localStorage.setItem('WALLET_NAME', WALLET_NAME); setAddOrBackBtn('sett')}} />
            </div>
          </AddNewMenuContainer>
        </Container>
      }
